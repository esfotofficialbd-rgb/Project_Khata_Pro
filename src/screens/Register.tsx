  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
         ...
         // Attempt explicit insert (fallback for Trigger), but don't fail if it errors (e.g., duplicate or RLS)
         // The Database Trigger 'on_auth_user_created' handles the actual creation reliably.
         await supabase.from('profiles').upsert(newProfile).catch(e => console.log("Manual profile sync skipped (handled by trigger)"));
         ...
         if (data.session) {
             ...
             // No session (maybe email confirmation required)
             // Try to force sign in to check if session can be established
             try {
                 ...
                 toast.success('অ্যাকাউন্ট তৈরি হয়েছে!', 'লগইন করুন।');
                 navigate('/login');
             }
         }
      }
      ...
      if (msg.includes("already registered")) msg = "এই ইমেইল দিয়ে ইতিমধ্যে একটি অ্যাকাউন্ট আছে। লগইন করুন।";
      setError(msg);
