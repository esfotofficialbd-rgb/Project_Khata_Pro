...
import { Bell, FileText, Calculator, CalendarClock, Building2, X, Send, Save, CheckCircle, Calendar, Clock, MapPin, Briefcase, Wallet, ChevronRight } from 'lucide-react';
...
export const WorkerHome = () => {
  ...
  return (
    <div className="p-4 space-y-6 pb-24 relative bg-slate-50 dark:bg-slate-950 min-h-screen">
      ...
      <div className="bg-gradient-to-br from-emerald-600 to-teal-800 rounded-3xl p-6 text-white shadow-xl shadow-emerald-200 dark:shadow-none relative overflow-hidden h-48 flex flex-col justify-between">
        {/* Background Pattern */}
        <div className="absolute top-0 right-0 w-48 h-48 bg-white opacity-5 rounded-full -mr-16 -mt-16 pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 w-32 h-32 bg-black opacity-10 rounded-full -ml-10 -mb-10 pointer-events-none"></div>

        <div className="relative z-10 flex justify-between items-start">
           <div>
              <p className="text-emerald-100 text-xs font-bold uppercase tracking-widest mb-1">{t('current_due')}</p>
              <h1 className="text-4xl font-bold tracking-tight">৳ {user.balance.toLocaleString()}</h1>
           </div>
        ...
        <div className="relative z-10 flex justify-between items-end">
           <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-bold tracking-wide">{companyName}</p>
           </div>
           <button 
             onClick={handleMessageOwner}
             className="bg-white text-emerald-700 px-4 py-2 rounded-xl text-xs font-bold shadow-sm flex items-center gap-1.5 hover:bg-emerald-50 transition-colors"
           >
             <Bell size={14} />
             {t('message_owner')}
           </button>
      ...
      {!isPresentToday && (
         <button 
            ...
            className="w-full bg-white dark:bg-slate-900 border-2 border-blue-500 dark:border-blue-600 p-1 rounded-2xl shadow-lg shadow-blue-100 dark:shadow-none active:scale-[0.98] transition-transform animate-pulse-slow"
         >
            <div className="bg-blue-500 dark:bg-blue-600 rounded-xl p-4 flex items-center justify-between">
               <div className="flex items-center gap-4 text-white">
                  ...
                  <div className="text-left">
                     ...
                     <p className="text-xs text-blue-100">কাজ শুরু করতে এখানে ট্যাপ করুন</p>
                  </div>
               </div>
      ...
      {isPresentToday && (
         <div className="w-full bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-900 rounded-2xl p-5 flex items-center gap-4">
            <div className="bg-emerald-100 dark:bg-emerald-900/50 p-3 rounded-full text-emerald-600 dark:text-emerald-400">
               <CheckCircle size={24} />
            </div>
            <div>
               <p className="font-bold text-slate-800 dark:text-white">{t('attendance_done')}</p>
               <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{t('attendance_recorded')}</p>
            </div>
         </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col justify-center items-center gap-1">
          <p className="text-3xl font-bold text-slate-800 dark:text-white">{totalDays}</p>
          <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">{t('total_work_days')}</p>
        </div>
        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col justify-center items-center gap-1">
          <p className="text-3xl font-bold text-slate-800 dark:text-white">৳ {(totalEarned).toLocaleString()}</p>
      ...
      {/* Quick Actions Grid */}
      <div>
        <h3 className="font-bold text-slate-800 dark:text-slate-200 mb-3 px-1 text-sm uppercase tracking-wider">{t('quick_actions')}</h3>
        <div className="grid grid-cols-2 gap-3">
          <button 
            onClick={() => setActiveModal('advance')}
            className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-3 active:scale-95 transition-transform"
          >
            ...
            <div className="text-left">
              ...
              <p className="text-[9px] text-slate-400">বেতন আবেদন</p>
            </div>
          </button>
          ...
          >
            ...
            <div className="text-left">
              ...
              <p className="text-[9px] text-slate-400">ছুটি নিন</p>
            </div>
          </button>
          ...
          <button 
            ...
            className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-3 active:scale-95 transition-transform"
          >
            <div className="bg-orange-50 dark:bg-orange-900/30 p-3 rounded-full text-orange-600 dark:text-orange-400">
               <Calculator size={20} />
            </div>
            <div className="text-left">
              <p className="font-bold text-slate-700 dark:text-white text-sm">{t('ot_calculator')}</p>
              <p className="text-[9px] text-slate-400">OT হিসাব</p>
            </div>
          </button>
          ...
          >
            ...
            <div className="text-left">
              ...
              <p className="text-[9px] text-slate-400">নোট রাখুন</p>
            </div>
          </button>
        </div>
      </div>
               ...
               <form onSubmit={handleAttendanceSubmit} className="space-y-4">
                  ...
                  <div className="max-h-60 overflow-y-auto space-y-2">
                     {projects.filter(p => p.status === 'active').map(project => (
                        <label 
                           key={project.id} 
                           className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all ${selectedProjectId === project.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30' : 'border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                        >
                           <input 
                              type="radio" 
                              name="project" 
                           ...
                           <div>
                              ...
                              <p className="text-[10px] text-slate-500 dark:text-slate-400 flex items-center gap-1"><MapPin size={10}/> {project.location}</p>
                           </div>
                        </label>
                     ))}
                  </div>
                  ...
                  <button 
                     ...
                     className="w-full py-4 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white rounded-2xl font-bold shadow-lg shadow-blue-200 dark:shadow-none mt-2 transition-all active:scale-95 flex items-center justify-center gap-2"
                  >
                     <Send size={20} />
                     রিকোয়েস্ট পাঠান
                  </button>
         ...
         <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2rem] w-full max-w-sm relative z-10 p-6 shadow-2xl animate-scale-up border border-slate-100 dark:border-slate-800">
               ...
               <form onSubmit={handleAdvanceSubmit} className="space-y-6">
                  <div className="bg-emerald-50 dark:bg-emerald-900/20 p-5 rounded-2xl text-center border border-emerald-100 dark:border-emerald-900/30">
              ...
              <form onSubmit={handleLeaveSubmit} className="space-y-4">
                 <div>
                    ...
                    <div className="relative">
                       ...
                       <input 
                         ...
                         className="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-purple-500 text-sm text-slate-900 dark:text-white font-medium"
                       />
                    </div>
                 </div>
                 ...
                 <div>
                    ...
                    <textarea 
                      ...
                      className="w-full p-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-purple-500 text-sm text-slate-900 dark:text-white h-24 resize-none font-medium"
                    />
                 </div>
      ...
      {activeModal === 'calc' && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
           ...
           <div className="bg-white dark:bg-slate-900 rounded-[2rem] w-full max-w-sm relative z-10 p-6 shadow-2xl animate-scale-up border border-slate-100 dark:border-slate-800">
              ...
              <div className="bg-orange-50 dark:bg-orange-900/20 p-5 rounded-2xl mb-6 text-center border border-orange-100 dark:border-orange-900/30">
                 ...
                 <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1 font-medium">প্রতি ঘণ্টা রেট: ৳ {Math.round(hourlyRate)}</p>
              </div>
                 ...
                 <div>
                    ...
                    <div className="relative">
                       ...
                       <input 
                         ...
                         className="w-full pl-11 pr-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-orange-500 text-sm text-slate-900 dark:text-white font-medium"
                       />
                    </div>
                 </div>

                 {otHours && (
                    <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-2xl border border-green-100 dark:border-green-800 text-center animate-scale-up">
                       <p className="text-xs text-green-700 dark:text-green-400 font-bold mb-1 uppercase">সম্ভাব্য আয়</p>
                       <p className="text-3xl font-bold text-green-600 dark:text-green-400">৳ {calculatedOtAmount}</p>
                    </div>
                 )}
              </div>
           </div>
           ...
           <div className="bg-white dark:bg-slate-900 rounded-[2rem] w-full max-w-sm relative z-10 p-6 shadow-2xl animate-scale-up border border-slate-100 dark:border-slate-800">
              ...
              <div className="space-y-4">
                 <textarea 
                    ...
                    className="w-full p-4 bg-blue-50/50 dark:bg-slate-800 border border-blue-100 dark:border-slate-700 rounded-2xl outline-none focus:border-blue-500 text-sm text-slate-800 dark:text-slate-200 h-48 resize-none leading-relaxed font-medium"
                 />
