...
import { MapPin, Search, Ruler, Clock, Coins, X, CheckCircle, Calculator, User, Phone, Send, Plus, ArrowRight, Building } from 'lucide-react';
...
export const Projects = () => {
  ...
  const inputClass = "w-full p-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-blue-500 text-sm font-medium text-slate-900 dark:text-white placeholder-slate-400 transition-all";
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-24 relative">
      ...
      <div className="bg-white dark:bg-slate-900 sticky top-0 z-20 px-4 pt-4 pb-2 shadow-sm border-b border-slate-100 dark:border-slate-800">
         <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-slate-800 dark:text-white">প্রজেক্ট তালিকা</h2>
            <button 
               onClick={() => setIsModalOpen(true)}
               className="bg-blue-600 text-white px-4 py-2.5 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-lg shadow-blue-200 dark:shadow-none hover:bg-blue-700 transition-colors"
            >
      ...
      <div className="p-4 space-y-4">
        {filteredProjects.length === 0 ? (
          <div className="text-center py-20">
             <div className="bg-slate-100 dark:bg-slate-800 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-4 border-4 border-white dark:border-slate-900 shadow-sm">
                <Clock size={32} className="text-slate-400" />
             </div>
             <p className="text-slate-500 dark:text-slate-400 font-medium">কোন প্রজেক্ট পাওয়া যায়নি</p>
          </div>
        ) : (
          filteredProjects.map((project) => {
            const progress = project.budget_amount > 0 ? (project.current_expense / project.budget_amount) * 100 : 0;
            let typeConfig = { label: 'ফিক্সড', icon: Coins, bg: 'bg-blue-50 dark:bg-blue-900/30', text: 'text-blue-600 dark:text-blue-400' };
            if (project.project_type === 'daily') typeConfig = { label: 'ডেইলি', icon: Clock, bg: 'bg-orange-50 dark:bg-orange-900/30', text: 'text-orange-600 dark:text-orange-400' };
            if (project.project_type === 'sqft') typeConfig = { label: 'স্কয়ার ফিট', icon: Ruler, bg: 'bg-purple-50 dark:bg-purple-900/30', text: 'text-purple-600 dark:text-purple-400' };
                ...
                <div className="flex gap-4 items-start">
                   ...
                   <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start">
                         <div>
                            <h3 className="font-bold text-slate-800 dark:text-white text-lg leading-tight mb-1 truncate">{project.project_name}</h3>
                            <p className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1">
                               <MapPin size={10} /> {project.location}
                            </p>
                         </div>
                         <div className={`px-2 py-1 rounded-lg text-[10px] font-bold ${typeConfig.bg} ${typeConfig.text} flex items-center gap-1`}>
                      ...
                      <div className="mt-4">
                         <div className="flex justify-between text-xs mb-1.5">
                            <span className="text-slate-500 font-bold">খরচ</span>
                         ...
                         {project.project_type !== 'daily' ? (
                           ...
                           <div className="w-full bg-orange-50 dark:bg-orange-900/20 rounded-lg p-2 text-center border border-orange-100 dark:border-orange-900/30">
                              <p className="text-[10px] text-orange-600 dark:text-orange-400 font-bold">ডেইলি বেসিস প্রজেক্ট</p>
                           </div>
                         )}
                      </div>
                   </div>
